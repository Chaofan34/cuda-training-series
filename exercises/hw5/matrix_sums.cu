...
{
  // 分解策略: 每个block一行
  int idx = blockIdx.x; // create typical 1D thread index from built-in variables
  if (idx < ds)
  {
    ...
    for (int i = tid; i < ds; i += blockDim.x)
    {
      sdata[tid] += A[idx * ds + i];
    }
...
int main()
{
  ...
  {
    auto t = TimeMonitor("column sums");
